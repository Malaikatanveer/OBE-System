...
public:
    ...
    void addprogram()
    {
        ...
        if (choice == 1)
        {
            ...
            printAllPrograms();
            char name[20];
            cout << "Enter existing program name you want to add";
            cin.getline(name, 20);
            cout << endl;
            int len = strlen(name);
            for (std::vector<Program*> ::iterator it = Allprograms.begin(); it != Allprograms.end(); ++it)
    ...
    {
        ofstream f1;
        f1.open("academicofficer.txt", ios::out);
        if (f1.is_open())
        {
...
public:
    ...
    void printEvaluation() {
        ...
        cout << "--------------------------------------------------------------------------------------" << endl;
        for (std::vector<Evaluation*> ::iterator it = evaluationHandled.begin(); it != evaluationHandled.end(); ++it) {
            cout << (*it)->geteId() << " " << (*it)->getTotalMarks() << endl;
        }
        cout << "--------------------------------------------------------------------------------------" << endl;
    }
    ...
    void printTopic() {
        ...
        cout << "--------------------------------------------------------------------------------------" << endl;
        for (std::vector<Topic*> ::iterator it = topicHandled.begin(); it != topicHandled.end(); ++it) {
            cout << (*it)->getTopicid()<<" "<<(*it) ->getname()<< endl;
        }
        cout << "--------------------------------------------------------------------------------------" << endl;
    }
...
//    address addr("50", "Awesome avenue", "Silver city");
  ...
  //  academicOfficer a1("CS", "Ahsan", 3, d, addr, "M");
    //Program p2(1,"CPS");
    Program p1(1,"Civil Engineering");
    Course c1(1,"Architecture", 3);
    CLO clo1(1, "Strong buildings");
    PLO plo1(1, "Create Buildings");
    c1.addCLO(&clo1);
    p1.addPLO(&plo1);
    p1.addCourse(&c1);
   Allprograms.push_back(&p1);
   AllPLOS.push_back(&plo1);
   ...
   // LOADING ACADEMIC OFFICER'S DATA
    while (1)
    {
        ...
        if (loginby == 1)
        {
            ...
            while (choice != 4)
            {
                cout << "[1]Manage Evaluations, [2]Manage Topics, [3]Save All, [4]Log Out ";
                cin >> choice;
                cin.ignore();
                if (choice == 1)                    
                    t1->manageEvaluation();
                ...
                {
                    cout << "Prinitng all teacher's data: " << endl;                  
                    t1->printEvaluation();
                    t1->printTopic();
        ...
        else if (loginby == 2)
        {
            ...
            while (choice != 7)
            {
                cout << "[1]Manage Programs, [2]Manage Courses, [3]Manage PLO, [4]Manage CLO, [5]Save All, [6]Print all global lists [7]Log Out ";
                cin >> choice;
                cin.ignore();
                if (choice == 1)
                    a1->managePrograms();
                ...
                {
                    cout << "Prinitng all academic officer's data: " << endl;
                    a1->printCLOS();
                    a1->printcourses();
                    a1->printPLOS();
                    a1->printprograms();
        ...
        while (cho != 1 && cho != 2)
        {
            cout << "Do you want to exit the system? [1]Yes [2]No";
            cin >> cho;
            cout << endl;
            if (cho == 1)
            {
